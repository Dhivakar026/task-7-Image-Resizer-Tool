...
Resizes all images in a folder (preserving aspect ratio by default).
Saves resized images to an output folder. Optionally converts format.

Usage:
    python image_resizer.py
    python image_resizer.py --input input_images --output output_images --width 800 --height 800 --format PNG --pad

Features:
- Preserves aspect ratio using thumbnail()
- Optional padding to produce exact target dimensions
- Keeps original format by default, or convert using --format
- Handles common formats including JPG, PNG, WEBP
"""
import os
import argparse
